...
def add_svo_filter_to_speclite(observatory, instrument, ffilter, update=False):
    """Download an SVO filter file and then add it to the user library :param
    observatory:
        ...
        try:
            ...
            with h5py.File(get_speclite_filter_library(), "a") as f:
                ...
                sub_grp.create_dataset(
                    "wavelength",
                    data=(data["Wavelength"] * waveunit.to("Angstrom")),
                    compression="gzip",
                )

                sub_grp.create_dataset(
                    "transmission", data=data["Transmission"], compression="gzip"
                )
...
def download_SVO_filters(filter_dict, update=False):
    ...
    for obs in observatories[::-1][50:]:
        ...
        #     obs =
        url = (
            "http://svo2.cab.inta-csic.es/svo/theory/fps/fps.php?Facility=%s"
            % obs.replace(" ", "%20")
        )
                    ...
                    if obs in f.keys():
                        if to_valid_python_name(instrument) in f[obs].keys():
                            if (
                                ...
                                in f[obs][to_valid_python_name(instrument)].keys()
                            ):
                                go = False
