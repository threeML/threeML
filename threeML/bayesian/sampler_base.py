...
class SamplerBase(metaclass=abc.ABCMeta):
    def __init__(self, likelihood_model: Model, data_list: DataList, **kwargs):
        """The base class for all bayesian samplers. Provides a common
        interface to access samples and byproducts of fits.

        :param likelihood_model: the likelihood model
        :param data_list: the data list
        :param share_spectrum: (optional) Should the spectrum be shared
        ...
        if "share_spectrum" in kwargs:
            ...
            assert (
                type(self._share_spectrum) is bool
    ...
    def log_like_values(self) -> Optional[np.ndarray]:
        ...
        sampler while samplin  g from the posterior. If you need to find the
    ...
    def _update_free_parameters(self) -> None:
        """Update the dictionary of the current free parameters :return:"""
    ...
    def _log_like(self, trial_values) -> float:
        ...
        try:
            ...
            if not self._share_spectrum:
                ...
                # If the calculation for the input spectrum of one of the sources is
                # expensive we want to avoid calculating the same thing several times.

                # Precalc the spectrum for all different Ebin_in that are used in the
                # plugins
                precalc_fluxes = []
...
class UnitCubeSampler(SamplerBase):
    ...
    def _construct_unitcube_posterior(self, return_copy=False):
        """Here, we construct the prior and log.

        likelihood for multinest etc on the unit cube
        """

        # First update the free parameters (in case the user changed them after the
        ...
        if return_copy:
            ...
            # Give a test run to the prior to check that it is working. If it crashes
            # while multinest is going it will not stop multinest from running and
            # generate thousands of exceptions (argh!)
