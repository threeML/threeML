...
class BayesianAnalysis:
    def __init__(self, likelihood_model: Model, data_list: DataList, **kwargs):
        """Perform Bayesian analysis by passing your model and data. All free
        parameters must have priors set.

        :param likelihood_model: the likelihood model
        :param data_list: the list of datasets to use (normally an
    ...
    def _register_model_and_data(
        ...
        for (
            ...
            if not parameter.has_prior():
                log.error(
                    "You need to define priors for all free parameters before "
                    "instancing a Bayesian analysis - "
        ...
        for dataset in list(self._data_list.values()):
            ...
            for parameter_name, parameter in list(dataset.nuisance_parameters.items()):
                ...
                # because otherwise multiple instance
    ...
    def set_sampler(self, sampler_name: str = "default", **kwargs):
        ...
        :param share_spectrum: (optional) Option to share the spectrum
            calc between detectors with the same input energy bins
        """
        ...
        if sampler_name not in _available_samplers:
            log.error(
                f"{sampler_name} is not a valid/available sampler please choose from "
                f"[blue]{','.join(list(_available_samplers.keys()))}[/blue]"
            )
    ...
    def sample(self, quiet=False, **kwargs) -> None:
        ...
        algorithm we default parameters will be run
    ...
    def log_like_values(self) -> Optional[np.ndarray]:
        """Returns the value of the log_likelihood found by the bayesian
        sampler while sampling from the posterior. If you need to find the
        values of the parameters which generated a given value of the log.
        likelihood, remember that the samples accessible through the property
        .raw_samples are ordered in the same way as the vector returned by this
        method.
    ...
    def log_probability_values(self) -> Optional[np.ndarray]:
        """Returns the value of the log_probability (posterior) found by the
        bayesian sampler while sampling from the posterior. If you need to find
        the values of the parameters which generated a given value of the log.
        likelihood, remember that the samples accessible through the property
        .raw_samples are ordered in the same way as the vector returned by this
        method.
    ...
    def raw_samples(self) -> Optional[np.ndarray]:
        """Access the samples from the posterior distribution generated by the
        selected sampler in raw form (i.e., in the format returned by the
        sampler)
    ...
    def samples(self) -> Optional[Dict[str, np.ndarray]]:
        ...
        :return: a dictionary with the samples from the posterior
    ...
    def sampler(self) -> Optional[SamplerBase]:
        """Access the instance of the sampler used to sample the posterior
        distribution :return: an instance of the sampler."""
