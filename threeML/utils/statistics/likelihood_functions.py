...
def poisson_observed_poisson_background_xs(
    ...
    """Profile log-likelihood for the case when the observed counts are Poisson
    distributed, and the background counts are Poisson distributed as well
    (typical for X-ray analysis with aperture photometry).

    This has been derived by Keith Arnaud (see the Xspec manual, Wstat
    statistic)
    """

    # We follow Arnaud et al. (Xspec manual) in the computation, which means that at the
    # end we need to multiply by (-1) as he computes the -log(L), while we need log(L).
    # Also, he multiplies -log(L) by 2 at the end to make it converge to chisq^2. We
    # don't do that to keep it a proper (profile) likelihood.
    ...
    # we regularize the log so it will not give NaN if expected_model_counts and
    # background_nuisance_parameter are both zero. For any good model this should also
    # mean observed_counts = 0, btw.
...
def poisson_observed_gaussian_background(
    ...
    for idx in range(n):
        ...
        # Now there are two branches: when the background is 0 we are in the normal
        # situation of a pure Poisson likelihood, while when the background is not zero
        # we use the profile likelihood

        # NOTE: bkgErr can be 0 only when also bkgCounts = 0
        # Also it is evident from the expression above that when bkgCounts = 0 and
        # bkgErr=0 also b=0
...
def half_chi2(y, yerr, expectation):  # pragma: no cover
    ...
    # can sum it with other likelihood functions.
