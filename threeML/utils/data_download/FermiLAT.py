...
from threeML.exceptions.custom_exceptions import TimeTypeNotKnown, TriggerDoesNotExist
from threeML.io.dict_with_pretty_print import DictWithPrettyPrint
from threeML.io.download_from_http import ApacheDirectory, RemoteDirectoryNotFound
from threeML.io.file_utils import if_directory_not_existing_then_make, sanitize_filename
...
_file_type_match = re.compile(r"gll_(\D{2,5})_bn\d{9}_v\d{2}\.\D{3}")
...
def merge_LAT_data(
    ...
    if outfile.exists():
        log.warning(
            ...
            "We assume you did not tamper with it, so we will return it instead of "
            "merging it again. If you want to redo the FT1 file again, remove it from "
            "the outdir"
        )
        return outfile
...
def download_LAT_data(
    ...
    """Download data from the public LAT data server (of course you need a
    working internet connection). Data are selected in a circular Region of
    Interest (cone) centered on the provided coordinates.

    Example:

    ```
    > download_LAT_data(195.6, -35.4, 12.0, '2008-09-16 01:00:23',
    '2008-09-18 01:00:23', time_type='Gregorian', destination_directory='my_new_data')
    ```

    :param ra: R.A. (J2000) of the center of the ROI
    :param dec: Dec. (J2000) of the center of the ROI
    :param radius: radius (in degree) of the center of the ROI (use a larger radius than
    what you will need in the analysis)
    ...
    :param data_type: type of data to download. Use Photon if you use Source or cleaner
    classes, Extended otherwise. Default is Photon.
    :param destination_directory: directory where you want to save the data (default:
    current directory)
    :param Emin: minimum photon energy (in MeV) to download (default: 30 MeV, must be
    between 30 and 1e6 MeV)
    :param Emax: maximum photon energy (in MeV) to download (default: 1e6 MeV, must be
    betwen 30 and 1e6 MeV )
    ...
    if Emin < fermiEmin:
        log.warning(
            f"Setting Emin from {Emin} to 30 MeV (minimum available energy for "
            "Fermi-LAT data)"
        )
        Emin = fermiEmin
    ...
    if Emin > fermiEmax:
        log.warning(
            f"Setting Emin from {Emin} to 1 TeV (maximum available energy for "
            "Fermi-LAT data)"
        )
        Emin = fermiEmax
    ...
    if Emax < fermiEmin:
        log.warning(
            f"Setting Emax from {Emax} to 30 MeV (minimum available energy for "
            "Fermi-LAT data)"
        )
        Emax = fermiEmin
    ...
    if Emax > fermiEmax:
        log.warning(
            f"Setting Emax from {Emax} to 1 TeV (maximum available energy for "
            "Fermi-LAT data)"
        )
        Emax = fermiEmax
    ...
    if Emin >= Emax:
        log.error(
            f"Minimum energy ({Emin}) must be less than maximum energy ({Emax}) for "
            "download."
        )
        raise ValueError()
    ...
    # Look if there are FT1 and FT2 files in the output directory matching this unique
    # ID
    ...
    if len(prev_downloaded_ft1s) > 0:
        ...
        # No need to look any further, if there is no FT1 file there shouldn't be any
        # FT2 file either
        pass
    ...
    if len(prev_downloaded_ft1s) > 0 and prev_downloaded_ft2 is not None:
        log.warning(
            f"Existing event file {prev_downloaded_ft1s} and Spacecraft file "
            f"{prev_downloaded_ft2} correspond to the same selection. "
            "We assume you did not tamper with them, so we will return those instead of"
            " downloading them again. If you want to download them again, remove them "
            "from the outdir"
        )
    ...
    try:
        ...
        estimated_time_for_the_query = re.findall(
            r"The estimated time for your query to complete is ([0-9]+) seconds",
            estimatedTimeLine,
        )[0]
        ...
        log.info(
            f"Estimated complete time for your query: {estimated_time_for_the_query} "
            "seconds"
        )
    ...
    for fits_file in downloaded_files:
        # Open the FITS file and write the unique key for this query, so that the
        # download will not be repeated if not necessary
...
class LAT_dataset:
    ...
    def make_LAT_dataset(
        ...
        eboundsFilename = os.path.join(
            destination_directory, "gll_%s_tr_bn%s_v00.rsp" % ("cspec", grb_name)
        )
...
def download_LLE_trigger_data(trigger_name, destination_directory="."):
    """Download the latest Fermi LAT LLE and RSP files from the HEASARC server.
    Will get the latest file versions. If the files already exist in your
    destination directory, they will be skipped in the download process. The
    output dictionary can be used as input to the FermiLATLLELike class.
    ...
    # Download only the lle, pt, cspec and rsp file (i.e., do not get all the png, pdf
    # and so on)
    pattern = r"gll_(lle|pt|cspec)_bn.+\.(fit|rsp|pha)"
