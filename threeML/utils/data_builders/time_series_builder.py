...
try:
    ...
    log.debug("ApolloX is available")

    has_apollox = True

except (ImportError):

    log.debug("ApolloX is unavailable")

    has_apollox = False


_allowed_response_type = [InstrumentResponse, InstrumentResponseSet, str]

try:

    import rball

    log.debug("rball is available")

    has_rball = True

    _allowed_response_type.append(rball.ResponseDatabase)

except (ImportError):

    log.debug("rball is unavailable")
...
class TimeSeriesBuilder(object):
    def __init__(
        ...
        **kwargs,
        ...
        if not issubclass(container_type, Histogram):
        ...
        self._drm_generator = None

        if response is not None:

            for rtype in _allowed_response_type:

                if isinstance(response, rtype):
        ...
        elif (has_rball) and isinstance(response, rball.ResponseDatabase):

            self._response = response.current_response
            self._drm_generator = response
    ...
    def to_spectrumlike(
        ...
        if not from_bins:
            ...
            if self._response is None:
                ...
                if has_rball and (self._drm_generator is not None):

                    log.debug(" found and rball")

                    return rball.RBallLike(
                        self._name,
                        observation=self._observed_spectrum,
                        background=this_background_spectrum,
                        response_database=self._drm_generator,
                    )

                elif not self._use_balrog:
    ...
    def from_konus_pha(
        ...
        """A builder to natively bin, view, and handle Konus-Wind PHA data.
