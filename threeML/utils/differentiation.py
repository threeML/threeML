...
def _get_wrapper(function, point, minima, maxima):
    ...
    # Find order of magnitude of each coordinate. If one of the coordinates is exactly
    # zero we need to treat it differently
    idx = point == 0.0

    orders_of_magnitude = np.zeros_like(point)
    orders_of_magnitude[idx] = 1.0
    orders_of_magnitude[~idx] = 10 ** np.ceil(np.log10(np.abs(point[~idx])))
    ...
    for i in range(n_dim):
        ...
        # distance to either boundary. 1/2 of that factor is due to the fact that
        # numdifftools uses twice the delta to compute the differential, and the 0.5 is
        # due to the fact that we don't want to go exactly equal to the boundary
    ...
    def wrapper(x):
        ...
        try:
            ...
            raise CannotComputeHessian(
                f"Cannot compute Hessian, parameters out of bounds at {scaled_back_x}"
            )
