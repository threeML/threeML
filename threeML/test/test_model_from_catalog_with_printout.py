...
def do_the_test(cat_name):
    ...
    for source in gta.get_sources():
        ...
        log.info(f"CATALOG_READ_TEST {cat_name}, {name}" )
        
        if np.allclose( f_fermipy, fa_fits) and np.allclose(f_fermipy, fa_cat):
            log.info("CATALOG_READ_TEST 1 passed")
        else:
            log.info("CATALOG_READ_TEST 1 failed")
            log.info(f"f_fermipy: {f_fermipy}")
            log.info(f"fa_cat: {fa_cat}")
            log.info(f"fa_fits: {fa_fits}")

        if cat_name == "4FGL-DR3":
            if np.allclose(f_fermipy, fa_vo):
                log.info("CATALOG_READ_TEST 2 passed")
            else:
                log.info("CATALOG_READ_TEST 2 failed")
                log.info(f"f_fermipy: {f_fermipy}")
                log.info(f"fa_vo: {fa_vo}")
        ...
        if type( model_cat[astro_name] ) == PointSource:
            ...
            if (pos_fits.separation(pos_cat).value < 1e-3) and (pos_fits.separation(pos_fermipy).value < 1e-3):
                log.info("CATALOG_READ_TEST 3 passed")
            else:
                log.info("CATALOG_READ_TEST 3 failed")
                log.info(f"pos_fits: {pos_fits}")
                log.info(f"pos_cat: {pos_cat}")
                log.info(f"pos_fermipy: {pos_fermipy}")
